...
app = Flask(__name__)
app.secret_key = "DalE_pUmAS_dAlE"  # Replace with your own secret key, needed for session management
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = beta0 + beta1 * X + mu + np.sqrt(sigma2) * np.random.randn(N, 1)  # Replace with code to generate Y
    ...
    for _ in range(S):
        ...
        X_sim = np.random.rand(N, 1)  # Generate random values for X between 0 and 1
        Y_sim = beta0 + beta1 * X_sim + mu + np.sqrt(sigma2) * np.random.randn(N, 1)  # Generate Y values with normal error
...
def hypothesis_test():
    ...
    plt.hist(simulated_stats, bins=20, alpha=0.7, color='gray', label='Simulated Values')
    ...
    plt.xlabel("Value")
    plt.ylabel("Frequency")
    plt.title("Histogram of Simulated Statistics with Hypothesis Test")
    plt.legend()
...
def confidence_interval():
    ...
    mean_estimate = np.mean(estimates)  # Calculate the mean of the simulated estimates
    std_estimate = np.std(estimates, ddof=1)  # Calculate the standard deviation of the simulated estimates


    # TODO 15: Calculate confidence interval for the parameter estimate
    # Use the t-distribution and confidence_level
    alpha = 1 - confidence_level / 100  # Calculate the significance level
    t_critical = t.ppf(1 - alpha / 2, df=S - 1)  # t critical value for two-tailed test
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(estimates, np.zeros_like(estimates), color="gray", alpha=0.5, s=50, label="Simulated Estimates")
    plt.plot(mean_estimate, 0, 'bo', label="Mean Estimate")
    plt.hlines(0, ci_lower, ci_upper, colors="green", linewidth=5, label=f"{confidence_level}% Confidence Interval")  # Solid and thicker line
    plt.axvline(true_param, color="red", linestyle="--", label="True Parameter")
